...
---- 数据下发
    ...
    AddClientModRPCHandler("moonlightcoda_rpc_namespace","pushevent.server2client.1",function(inst,data)
        pushevent_server2client(inst,data)
    end)
    AddClientModRPCHandler("moonlightcoda_rpc_namespace","pushevent.server2client.2",function(inst,data)
        pushevent_server2client(inst,data)
    end)
    AddClientModRPCHandler("moonlightcoda_rpc_namespace","pushevent.server2client.3",function(inst,data)
        pushevent_server2client(inst,data)
    end)
    AddClientModRPCHandler("moonlightcoda_rpc_namespace","pushevent.server2client.4",function(inst,data)
        pushevent_server2client(inst,data)
    end)
    AddClientModRPCHandler("moonlightcoda_rpc_namespace","pushevent.server2client.5",function(inst,data)
        pushevent_server2client(inst,data)
    end)
-- SendModRPCToClient(CLIENT_MOD_RPC["moonlightcoda_rpc_namespace"]["pushevent.server2client"],inst.userid,inst,json.encode(json_data))
...
---- 数据上传
    ...
    AddModRPCHandler("moonlightcoda_rpc_namespace", "pushevent.client2server.1", function(player_inst,inst,data_json) ----- Register on the server
        pushevent_client2server(player_inst,data_json)
    end)
    AddModRPCHandler("moonlightcoda_rpc_namespace", "pushevent.client2server.2", function(player_inst,inst,data_json) ----- Register on the server
        pushevent_client2server(player_inst,data_json)
    end)
    AddModRPCHandler("moonlightcoda_rpc_namespace", "pushevent.client2server.3", function(player_inst,inst,data_json) ----- Register on the server
        pushevent_client2server(player_inst,data_json)
    end)
    AddModRPCHandler("moonlightcoda_rpc_namespace", "pushevent.client2server.4", function(player_inst,inst,data_json) ----- Register on the server
        pushevent_client2server(player_inst,data_json)
    end)
    AddModRPCHandler("moonlightcoda_rpc_namespace", "pushevent.client2server.5", function(player_inst,inst,data_json) ----- Register on the server
        pushevent_client2server(player_inst,data_json)
    end)
-- -- SendModRPCToServer(MOD_RPC["moonlightcoda_rpc_namespace"]["pushevent.client2server"],self.inst,json.encode(data_table))
