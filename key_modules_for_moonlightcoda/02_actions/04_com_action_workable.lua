...
local MCODA_COM_WORKABLE_ACTION = Action({priority = 10})   --- 距离 和 目标物体的 碰撞体积有关，为 0 也没法靠近。
MCODA_COM_WORKABLE_ACTION.id = "MCODA_COM_WORKABLE_ACTION"
MCODA_COM_WORKABLE_ACTION.strfn = function(act) --- 客户端检查是否通过,同时返回显示字段
    ...
    if doer and target and target.replica.mcoda_com_workable then
        local replica_com = target.replica.mcoda_com_workable or target.replica._.mcoda_com_workable
        if replica_com then
...
MCODA_COM_WORKABLE_ACTION.fn = function(act)    --- 只在服务端执行~
    local item = act.invobject
    local target = act.target
    local doer = act.doer
    ...
    if target and doer and target.components.mcoda_com_workable then
        local replica_com = target.replica.mcoda_com_workable or target.replica._.mcoda_com_workable
        if replica_com and replica_com:Test(doer,true) then
            return target.components.mcoda_com_workable:Active(doer)
        end
    end
    return false
end
AddAction(MCODA_COM_WORKABLE_ACTION)
...
--             table.insert(actions, ACTIONS.MCODA_COM_WORKABLE_ACTION)
--         end        
--     end
-- end)

AddComponentAction("SCENE", "mcoda_com_workable" , function(target, doer, actions, right_click)-------    建筑一类的特殊交互使用
    if doer and target then
            local mcoda_com_workable_com = target.replica.mcoda_com_workable or target.replica._.mcoda_com_workable
            if mcoda_com_workable_com and mcoda_com_workable_com:Test(doer,right_click) then
                table.insert(actions, ACTIONS.MCODA_COM_WORKABLE_ACTION)
            end        
    end
end)
AddComponentAction("INVENTORY", "mcoda_com_workable" , function(item, doer, actions, right_click)    -------    物品一类交互使用
    if doer and item then
            local mcoda_com_workable_com = item.replica.mcoda_com_workable or item.replica._.mcoda_com_workable
            if mcoda_com_workable_com and mcoda_com_workable_com:Test(doer,right_click) then
                table.insert(actions, ACTIONS.MCODA_COM_WORKABLE_ACTION)
            end
    end
...
AddStategraphActionHandler("wilson",ActionHandler(MCODA_COM_WORKABLE_ACTION,function(player)
    local creash_flag , ret = pcall(function()
        ...
        local replica_com = target and ( target.replica.mcoda_com_workable or target.replica._.mcoda_com_workable )
        if replica_com then
    ...
    if creash_flag == true then
        ...
        print("error in MCODA_COM_WORKABLE_ACTION ActionHandler")
        print(ret)
    end
    return "dolongaction"
end))
AddStategraphActionHandler("wilson_client",ActionHandler(MCODA_COM_WORKABLE_ACTION, function(player)    
    local creash_flag , ret = pcall(function()
        local target = player.bufferedaction.target or player.bufferedaction.invobject
        local ret_sg_action = "dolongaction"
        local replica_com = target and ( target.replica.mcoda_com_workable or target.replica._.mcoda_com_workable )
