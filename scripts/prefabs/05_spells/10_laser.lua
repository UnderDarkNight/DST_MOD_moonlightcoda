...
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
TUNING.MOONLIGHTCODA_SPELL_LASER_DAMAGE = 90  --- 默认伤害
...
local prefabs =
{
    "moonlightcoda_spell_laserscorch",
    "moonlightcoda_spell_lasertrail",
    "moonlightcoda_spell_laserhit",
}
...
local function DoDamage(inst, targets, skiptoss, skipscorch)
    ...
    if inst.AnimState ~= nil then
        ...
        if not skipscorch and TheWorld.Map:IsPassableAtPoint(x, 0, z, false) then
            SpawnPrefab("moonlightcoda_spell_laserscorch").Transform:SetPosition(x, 0, z)
        end

        local fx = SpawnPrefab("moonlightcoda_spell_lasertrail")
        ...
        if not targets[v] and v:IsValid() and
            ...
            if dsq_to_laser < range * range then
                ...
                elseif inst.components.combat:CanTarget(v) then
                    ...
                    SpawnPrefab("moonlightcoda_spell_laserhit"):SetTarget(v)
    ...
    if platform_hit then
        ...
        platform_hit.components.health:DoDelta(-1 * TUNING.MOONLIGHTCODA_SPELL_LASER_DAMAGE / 10)
    end
end
...
local function common_fn(isempty)
    ...
    inst.components.combat:SetDefaultDamage(TUNING.MOONLIGHTCODA_SPELL_LASER_DAMAGE)
...
local function scorchfn()
    ...
    inst._fade = net_byte(inst.GUID, "moonlightcoda_spell_laserscorch._fade", "fadedirty")
...
return Prefab("moonlightcoda_spell_laser", fn, assets, prefabs),
    Prefab("moonlightcoda_spell_laserempty", emptyfn, assets, prefabs),
    Prefab("moonlightcoda_spell_laserscorch", scorchfn, assets_scorch),
    Prefab("moonlightcoda_spell_lasertrail", trailfn, assets_trail),
    Prefab("moonlightcoda_spell_laserhit", hitfn)
